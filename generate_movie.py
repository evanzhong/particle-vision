...
for ROI in ROIS:
  ...
  color_frame = cv2.putText(box_cropped, f'Frame: {frame_num}', (0,len(box_cropped)-1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 2)

  text_color = (255, 255, 255)
  binary_frame = cv2.putText(seg.otsu_threshold_image(box_cropped), f'Frame: {frame_num}', (0,len(box_cropped)-1), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 2)

  both_frames = np.hstack((color_frame, cv2.cvtColor(binary_frame, cv2.COLOR_GRAY2RGB)))
  util.write_image(both_frames, f'frame_{frame_num}')
